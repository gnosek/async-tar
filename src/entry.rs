...
use crate::{other, Header, PaxExtensions};
use async_std::fs;
use async_std::io::prelude::SeekExt;
use async_std::io::{self, Error, ErrorKind, Read, ReadExt, SeekFrom};
...
impl SparseEntry {
    fn poll_read<R: Read + Unpin>(
        ...
        match self {
            SparseEntry::Data(s) => {
                let to_read = std::cmp::min(*s, out_buf.len());
                let res = ready!(reader.poll_read(cx, &mut out_buf[..to_read]));
                match res {
                    Ok(n) if n == *s => {
                        *self = SparseEntry::None;
                        pos.fetch_add(n as u64, Ordering::Relaxed);
                        Poll::Ready(Ok(n))
                    }
                    Ok(n) => {
                        *self = SparseEntry::Data(*s - n);
                        pos.fetch_add(n as u64, Ordering::Relaxed);
                        Poll::Ready(Ok(n))
                    }
            ...
            SparseEntry::Padding(s) => {
                let to_read = std::cmp::min(*s, out_buf.len());
                unsafe {
                    std::ptr::write_bytes(out_buf.as_mut_ptr(), 0, to_read);
                }
                if to_read == *s {
                    *self = SparseEntry::None;
                } else {
                    *self = SparseEntry::Padding(*s - to_read);
                }
                Poll::Ready(Ok(to_read))
            }
            SparseEntry::None => panic!("Read from zero-sized SparseEntry requested"),
        }
    }

    async fn write_all<R: Read + Unpin>(
        &mut self,
        reader: &mut R,
        pos: &Arc<AtomicU64>,
        file: &mut fs::File,
        ...
        match self {
            SparseEntry::Data(s) => {
                let reader = reader.take(*s as u64);
                let nwritten = io::copy(reader, file).await?;
                if nwritten == *s as u64 {
                    *self = SparseEntry::None;
                    pos.fetch_add(nwritten, Ordering::Relaxed);
                    Ok(())
                } else {
                    *self = SparseEntry::Data(*s - nwritten as usize);
                    pos.fetch_add(nwritten, Ordering::Relaxed);
                    Err(other("Failed to write entire file"))
                }
            }
            SparseEntry::Padding(s) => {
                let pos = SeekFrom::Current(*s as i64);
                let size = file.seek(pos).await?;
...
#[pin_project]
pub struct SparseReader<R: Read + Unpin> {
    #[pin]
    reader: R,
    pos: Arc<AtomicU64>,
    chunks: VecDeque<SparseEntry>,
}

impl<R: Read + Unpin> fmt::Debug for SparseReader<R> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("SparseReader")
            .field("chunks", &self.chunks)
            .finish()
    }
}

impl<R: Read + Unpin> Read for SparseReader<R> {
    fn poll_read(
        self: Pin<&mut Self>,
        cx: &mut Context,
        out_buf: &mut [u8],
        ...
        let chunk = match this.chunks.get_mut(0) {
            None => return Poll::Ready(Ok(0)),
            Some(chunk) => chunk,
        };

        let res = ready!(chunk.poll_read(cx, out_buf, this.reader, this.pos));
        if let SparseEntry::None = chunk {
...
impl<R: Read + Unpin> SparseReader<R> {
    pub(crate) fn new(reader: R, pos: Arc<AtomicU64>, chunks: VecDeque<SparseEntry>) -> Self {
        Self {
            reader,
            pos,
            chunks,
    ...
    pub(crate) fn new_single_chunk(reader: R, pos: Arc<AtomicU64>, size: usize) -> Self {
        let mut chunks = VecDeque::new();
        chunks.push_back(SparseEntry::Data(size));
        Self {
            reader,
            pos,
            chunks,
    ...
    async fn write_all(&mut self, file: &mut fs::File) -> io::Result<()> {
        loop {
            ...
            chunk.write_all(&mut self.reader, &self.pos, file).await?;
            if let SparseEntry::None = chunk {
    ...
    fn size_hint(&self, max: usize) -> usize {
        let mut sum = 0;
        for chunk in &self.chunks {
            match chunk {
                SparseEntry::None => (),
                SparseEntry::Data(s) => sum += *s,
                SparseEntry::Padding(s) => sum += *s,
            }
            if sum >= max {
                return max;
            }
    ...
    pub(crate) async fn read_all(&mut self) -> io::Result<Vec<u8>> {
        ...
        let cap = self.size_hint(128 * 1024);
...
impl<R: Read + Unpin> Entry<R> {
    ...
    pub fn path_bytes(&self) -> Cow<[u8]> {
        match self.fields.long_pathname {
            Some(ref bytes) => {
    ...
    pub fn link_name_bytes(&self) -> Option<Cow<[u8]>> {
        match self.fields.long_linkname {
            Some(ref bytes) => {
                if let Some(&0) = bytes.last() {
    ...
    pub async fn pax_extensions(&mut self) -> io::Result<Option<PaxExtensions<'_>>> {
        if self.fields.pax_extensions.is_none() {
            if !self.header().entry_type().is_pax_global_extensions()
                && !self.header().entry_type().is_pax_local_extensions()
            {
                return Ok(None);
            }
            self.fields.pax_extensions = Some(self.data.read_all().await?);
        }
        Ok(Some(pax_extensions(self.fields.pax_extensions.as_ref().unwrap())))
    }
    ...
    /// Internal helper for unpacking files
    async fn unpack_impl(
        &mut self,
        ...
        if kind.is_dir() {
            ...
            if let Ok(mode) = self.header().mode() {
                set_perms(dst, None, mode, self.fields.preserve_permissions).await?;
            }
        ...
        let mut f = async {
            ...
            self.data.write_all(&mut f).await?;
        ...
        if let Ok(mode) = self.header().mode() {
            set_perms(dst, Some(&mut f), mode, self.fields.preserve_permissions).await?;
        }
        if self.fields.unpack_xattrs {
        ...
        async fn set_xattrs<R: Read + Unpin>(me: &mut Entry<R>, dst: &Path) -> io::Result<()> {
        ...
        async fn set_xattrs<R: Read + Unpin>(_: &mut Entry<R>, _: &Path) -> io::Result<()> {
            Ok(())
        }
    }
    ...
    pub async fn unpack<P: AsRef<Path>>(&mut self, dst: P) -> io::Result<Unpacked> {
        self.unpack_impl(None, dst.as_ref()).await
    }
    ...
    pub async fn unpack_in<P: AsRef<Path>>(&mut self, dst: P) -> io::Result<bool> {
        ...
        self.unpack_impl(Some(&canon_target), &file_dst)
            .await
